# Checks if EMET is installed

$log = 'C:\ProgramData\PuppetLabs\scripts\puppet\logs\test_if_emet_is_installed.log'
...
If (Test-Path 'C:\Program Files (x86)\EMET 5.5')
{
    Write-Output "EMET is installed" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Write-Output "####################" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Exit 0
...
Else
{
    Write-Output "EMET is NOT installed, running correction script" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Write-Output "####################" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Exit 1
