# Test if admin account is disabled

$log = 'C:\ProgramData\PuppetLabs\scripts\puppet\logs\test_if_admin_is_disabled.log'
...
{
    Write-Output "$date : Account not active - Administrator" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Write-Output "####################" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Exit 0
...
Else
{
    Write-Output "$date : Account Active - Administrator -> Running correction script" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Write-Output "####################" | 
    Out-File -FilePath $log -Encoding ascii -Append
    Exit 1
